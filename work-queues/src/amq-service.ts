...
export default class AmqService {
   ...
   */
  public sendMessage(rawData: string, connection: Connection, durableQueue: boolean = false) {
    const queueName = this.queueName;
    return new Promise((resolve) => {
      console.log(`Sending data...`);
      const createChannel = promisify(connection.createChannel);
      connection.createChannel((err, channel: Channel) => {
        channel.assertQueue(queueName, {durable: durableQueue});
        const data: Buffer = new Buffer(rawData, "utf-8");
        channel.sendToQueue(queueName, data, { persistent: true });
